...
dat5<-as.data.frame(fread("usa_00043.csv/usa_00043.csv", colClasses=colClasses))
dat5[dat5$INCWELFR==99999,"INCWELFR"]<-0


dat6<-dat5%>%
  group_by(STATEFIP, YEAR)%>%
  dplyr::summarise(tot=sum(as.numeric(PERWT)),
                   child=(sum((AGE<18)*as.numeric(PERWT))),
                   ...
                   lessHS=sum((AGE>25)*(EDUCD<62)*PERWT) / sum((AGE>25)*PERWT),
